"""
Tang Primer 25K Platform Definition
"""
...
# Base IOs (from LiteX-Boards' sipeed_tang_nano_25k Platform) ----------------
...
_dock_io = [
    ...
    # Leds.
    ("user_led", 0, Pins("J1:17"), IOStandard("LVCMOS33")),  # READY.
    ("user_led", 1, Pins("J1:25"), IOStandard("LVCMOS33")),  # DONE.

    # Buttons (active-low on this design, keep name but expose as buttons).
    ("user_btn", 0, Pins("J1:37"), IOStandard("LVCMOS33")),
    ("user_btn", 1, Pins("J1:39"), IOStandard("LVCMOS33")),
...
def misterSDRAM(conn="j3"):
    ...
    return [
        ...
        ("sdram", 0,
            Subsignal("a", Pins(
                f"{conn}:37 {conn}:38 {conn}:39 {conn}:40 {conn}:28 "
                f"{conn}:25 {conn}:26 {conn}:23 {conn}:24 {conn}:21 "
                f"{conn}:36 {conn}:22 {conn}:19"
            )),
            Subsignal("dq", Pins(
...
def sipeedSDRAM(conn="j3"):
    """Sipeed SDR SDRAM module on connector `conn`."""
    return [
        ("sdram_clock", 0, Pins(f"{conn}:20"),
            IOStandard("LVCMOS33"),
            Misc("PULL_MODE=NONE DRIVE=16"),
